namespace :test do 
  task :prepare do 
    mkdir_p "Tests/FoundryTests.xcodeproj/xcshareddata/xcschemes"
  ...
  desc "Run the Foundry Tests for Mac OSX"
  task :osx => :prepare do
...
def apple_doc_command
  "/usr/local/bin/appledoc -o Docs/ -p Foundry -v #{spec_version} -c \"Foundry\" " +
...
def run_tests(scheme, sdk)
  sh("xcodebuild -workspace Foundry.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' -configuration Release clean test | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end
